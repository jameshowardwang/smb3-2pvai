...
void SSRVideoStreamReader::Init() {
	...
	Logger::LogInfo("[SSRVideoStreamReader::Init] Created video stream reader.");

	// open main file
	m_fd_main = open(m_filename_main.c_str(), O_RDWR | O_CLOEXEC);
	if(m_fd_main == -1) {
		Logger::LogError("[SSRVideoStreamReader::Init] Error: Can't open video stream file!");
	...
	if(ftruncate(m_fd_main, m_mmap_size_main) == -1) {
		Logger::LogError("[SSRVideoStreamReader::Init] Error: Can't resize video stream file!");
		throw SSRStreamException();
	}
	...
	if(m_mmap_ptr_main == MAP_FAILED) {
		Logger::LogError("[SSRVideoStreamReader::Init] Error: Can't memory-map video stream file!");
		throw SSRStreamException();
	}
	...
	for(unsigned int i = 0; i < GLINJECT_RING_BUFFER_SIZE; ++i) {
		...
		if(fd.m_fd_frame == -1) {
			Logger::LogError("[SSRVideoStreamReader::Init] Error: Can't open video frame file!");
			throw SSRStreamException();
		}
	}
...
void SSRVideoStreamReader::Free() {
	...
	Logger::LogInfo("[SSRVideoStreamReader::Init] Destroyed video stream reader.");
	...
	if(required_size > fd.m_mmap_size_frame) {
		...
		// check frame file size
		{
			...
			if(fstat(fd.m_fd_frame, &statinfo) == -1 || (size_t) statinfo.st_size < required_size) {
				Logger::LogError("[SSRVideoStreamReader::GetFrame] Error: Size of video frame file is incorrect!");
				throw SSRStreamException();
			}
		...
		if(fd.m_mmap_ptr_frame == MAP_FAILED) {
			Logger::LogError("[SSRVideoStreamReader::GetFrame] Error: Can't memory-map video frame file!");
			throw SSRStreamException();
		}
