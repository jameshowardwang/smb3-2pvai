...
void SSRVideoStreamWatcher::Init() {
	...
	if(mkdir(m_channel_directory.c_str(), (m_relax_permissions)? 0777 : 0700) == -1) {
		...
		if(errno != EEXIST) {
			Logger::LogError("[SSRVideoStreamWatcher::Init] Error: Can't create channel directory!");
		...
		if(lstat(m_channel_directory.c_str(), &statinfo) == -1) {
			Logger::LogError("[SSRVideoStreamWatcher::Init] Error: Can't stat channel directory!");
			throw SSRStreamException();
		}
		if(!S_ISDIR(statinfo.st_mode) || S_ISLNK(statinfo.st_mode)) {
			Logger::LogError("[SSRVideoStreamWatcher::Init] Error: Channel directory is not a regular directory!");
			throw SSRStreamException();
		}
		if(statinfo.st_uid == geteuid()) {
			if(chmod(m_channel_directory.c_str(), (m_relax_permissions)? 0777 : 0700) == -1) {
				Logger::LogError("[SSRVideoStreamWatcher::Init] Error: Can't set channel directory mode!");
				throw SSRStreamException();
			}
		} else {
			if(!m_relax_permissions) {
				Logger::LogError("[SSRVideoStreamWatcher::Init] Error: Channel directory is owned by a different user! "
	...
	if(m_fd_notify == -1) {
		Logger::LogError("[SSRVideoStreamWatcher::Init] Error: Can't initialize inotify!");
		throw SSRStreamException();
	}
	...
	if(inotify_add_watch(m_fd_notify, m_channel_directory.c_str(), IN_CREATE | IN_DELETE | IN_MOVED_FROM | IN_MOVED_TO) == -1) {
		Logger::LogError("[SSRVideoStreamWatcher::Init] Error: Can't watch channel directory!");
		throw SSRStreamException();
	}
#endif
	...
	try {
		...
		if(dir == NULL) {
			Logger::LogError("[SSRVideoStreamWatcher::Init] Error: Can't open channel directory!");
			throw SSRStreamException();
		}
		...
		for( ; ; ) {
			...
			// add the stream
			Logger::LogInfo(std::string("[SSRVideoStreamWatcher::Init] Added pre-existing stream ") + d->d_name);
...
void SSRVideoStreamWatcher::HandleChanges(AddCallback add_callback, RemoveCallback remove_callback, void* userdata) {
	...
	if(ioctl(m_fd_notify, FIONREAD, &len) == -1) {
		Logger::LogError("[SSRVideoStreamWatcher::GetChanges] Error: Can't get read length from inotify!");
		throw SSRStreamException();
	}
	if(len > 0) {
		...
		if(read(m_fd_notify, buffer.data(), buffer.size()) != (ssize_t) buffer.size()) {
			Logger::LogError("[SSRVideoStreamWatcher::GetChanges] Error: Can't read from inotify!");
			throw SSRStreamException();
		}
		...
		while(pos < buffer.size()) {
			...
			if(buffer.size() - pos < sizeof(inotify_event)) {
				Logger::LogError("[SSRVideoStreamWatcher::GetChanges] Error: Received partial event from inotify!");
				throw SSRStreamException();
			}
			...
			if(buffer.size() - pos < event->len) {
				Logger::LogError("[SSRVideoStreamWatcher::GetChanges] Error: Received partial name from inotify!");
				throw SSRStreamException();
			}
			...
			if(event->mask & (IN_CREATE | IN_MOVED_TO)) {
				if(std::find(m_streams.begin(), m_streams.end(), stream) == m_streams.end()) {
					Logger::LogInfo(std::string("[SSRVideoStreamWatcher::GetChanges] Added stream ") + stream.m_stream_name);
					m_streams.push_back(stream);
			...
			if(event->mask & (IN_DELETE | IN_MOVED_FROM)) {
				...
				if(p != m_streams.end()) {
					Logger::LogInfo(std::string("[SSRVideoStreamWatcher::GetChanges] Removed stream ") + stream.m_stream_name);
	...
	for(unsigned int j = m_streams.size(); j > 0; ) {
		...
		unlink(filename.c_str());
		Logger::LogInfo(std::string("[SSRVideoStreamWatcher::Init] Deleted abandoned stream ") + m_streams[j].m_stream_name);
